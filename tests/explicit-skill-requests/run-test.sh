...
if [ -z "$SKILL_NAME" ] || [ -z "$PROMPT_FILE" ]; then
    ...
    echo "Example: $0 subagent-driven-analysis ./prompts/subagent-driven-analysis-please.txt"
    exit 1
fi

# Get the directory where this script lives
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# Get the datapowers plugin root (two levels up)
PLUGIN_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"

TIMESTAMP=$(date +%s)
OUTPUT_DIR="/tmp/datapowers-tests/${TIMESTAMP}/explicit-skill-requests/${SKILL_NAME}"
mkdir -p "$OUTPUT_DIR"
...
# Create a minimal CSV dataset for analysis tests
python3 - <<PYEOF
import csv, random
random.seed(42)
rows = [[i, random.randint(18, 70), random.randint(1, 60), round(random.uniform(20, 200), 2), random.randint(0, 10), int(random.random() > 0.8)] for i in range(100)]
with open("$PROJECT_DIR/data/customers.csv", "w", newline="") as f:
    writer = csv.writer(f)
